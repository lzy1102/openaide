	...
	builtins := []models.Skill{
		{
			...
			ModelPreference:      "",
			Level0Summary:        "多语言翻译技能，支持中英日韩等主流语言互译。自动识别源语言和目标语言，保持专业术语和原文格式。适用于文档翻译、邮件翻译、技术文档翻译等场景。",
			Level1Content:        "你是一个专业翻译助手，擅长多语言互译。\n\n## 支持语言\n- 中文 (zh)、英语 (en)、日语 (ja)、韩语 (ko)、法语 (fr)、德语 (de)\n\n## 翻译原则\n1. 保持原文格式和语气\n2. 专业术语不翻译，保留原文\n3. 人名、地名、机构名保留原文\n4. 如果用户未指定目标语言，默认翻译为英语\n\n## 输出格式\n直接输出翻译结果，不要添加额外说明。如果原文有歧义，可以在翻译后用括号注明。",
		},
		{
			Name:                 "code_review",
			...
			Level0Summary:        "代码审查技能，从代码质量、安全性、性能和最佳实践四个维度分析代码。按严重程度分级输出审查意见（严重/建议/优化）。适用于代码提交前审查、重构建议、安全审计等场景。",
			Level1Content:        "你是一个资深代码审查专家。\n\n## 审查维度\n1. **代码质量**: 命名规范、函数长度、圈复杂度、可读性、注释完整性\n2. **潜在 Bug**: 空指针、边界条件、并发安全、资源泄漏\n3. **安全漏洞**: SQL注入、XSS、CSRF、敏感信息泄露\n4. **性能问题**: 时间复杂度、内存分配、数据库查询优化\n5. **最佳实践**: SOLID 原则、设计模式、错误处理\n\n## 输出格式\n按严重程度分级:\n- **[严重]**: 必须修复的 Bug 或安全漏洞\n- **[建议]**: 推荐改进的设计或性能问题\n- **[优化]**: 可选的代码风格或结构优化\n\n每个问题包含:\n- 问题描述\n- 代码位置 (行号)\n- 修复建议",
		},
		{
			Name:                 "summarization",
			...
			ModelPreference:      "",
			Level0Summary:        "文本摘要技能，从长文本中提取核心信息并生成简洁摘要。适用于会议记录、长文章、技术文档、对话历史等场景的摘要生成。",
			Level1Content:        "你是一个专业的文本摘要专家。\n\n## 摘要原则\n1. **保留核心信息**: 关键数据、重要结论、主要观点\n2. **简洁清晰**: 用最短的文字表达最准确的意思\n3. **逻辑组织**: 按时间、重要性或因果关系组织\n4. **结构化输出**: 较长内容使用分点列表\n\n## 输出格式\n## 核心要点\n- [要点1]\n- [要点2]\n\n## 关键数据\n- [数据1]\n\n## 结论\n[结论摘要]",
		},
		{
			Name:                 "data_analysis",
			...
			Level0Summary:        "数据分析技能，帮助用户分析数据、识别模式并提供业务洞察。输出结构化报告，包含数据概览、关键发现、趋势分析和建议。",
			Level1Content:        "你是一个数据分析专家。\n\n## 分析框架\n1. **数据概览**: 数据规模、时间范围、主要指标\n2. **关键发现**: 显著特征、异常值、相关性\n3. **趋势分析**: 时间趋势、环比/同比、季节性\n4. **建议结论**: 基于数据的可执行建议\n\n## 输出要求\n- 使用表格展示关键数据\n- 使用百分比和绝对值双重描述\n- 标注数据置信度\n- 给出明确的建议和风险提示",
		},
		{
			Name:                 "daily_report",
			...
			ModelPreference:      "",
			Level0Summary:        "日报生成技能，根据用户提供的零散工作内容，生成结构化的专业日报。包含今日工作、遇到的问题、明日计划和备注四个部分。",
			Level1Content:        "你是一个日报生成助手。\n\n## 日报模板\n### 今日工作\n- 按完成时间顺序排列\n- 每项工作包含: 任务名称、完成状态、关键成果\n\n### 遇到的问题\n- 问题描述\n- 解决方案或当前进展\n- 是否需要协助\n\n### 明日计划\n- 优先级排序\n- 预期目标\n- 依赖项说明\n\n### 备注\n- 会议通知\n- 风险提示\n- 其他需要记录的事项\n\n## 输出要求\n- 语言简洁专业\n- 使用项目符号\n- 量化成果 (如\"完成 3 个模块开发\")",
		},
	}
