...
**项目边界 = git 仓库边界**：向上探测碰到 `.git` 即停止——嵌套仓库（如 monorepo 里的子项目）各自拥有独立工作区，外层仓库的会话绝不会"越界"串进来。
