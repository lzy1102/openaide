...
codeindex:
  ...
  min_score: 0.3        # 注入最低相似度（0 = 默认 0.3）
  score_mode: fixed     # 阈值策略: fixed(默认) / relative / combined
  score_ratio: 0.6      # relative/combined 的相对阈值比例（0 = 默认 0.6）；低于阈值的检索结果不注入
...
- **代码注入阈值**：coding/debugging 任务注入的代码 chunk 需满足相似度阈值。`codeindex.score_mode` 选择策略：
  - `fixed`（默认）：相似度 ≥ `min_score`（默认 0.3）
  - `relative`：相似度 ≥ 最高分 × `score_ratio`（默认 0.6），自适应 embedding 模型分布
  - `combined`：先按 relative 过滤，且最高分低于 `min_score` 时全部丢弃
  注意各后端 Score 已统一为相似度（越大越好），redis 返回的距离已在内部归一化。
