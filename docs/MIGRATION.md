...
> 基于与主流 Agent（Claude Code、Cursor、Devin）的对比分析，以下改进为最高优先级
> 目标：缩小功能差距，保持核心优势（本地、隐私、零依赖、开源）

### 13.1 P0 改进项

| 改进项 | 当前状态 | 目标状态 | 影响 | 难度 | 时间 |
|--------|----------|----------|------|------|------|
| **Git 集成** | 仅能执行命令 | 智能解析 + 提交建议 | 🔴 严重 | 中 | 2 周 |
| **代码索引** | 无 | 符号索引 + 调用分析 | 🔴 致命 | 高 | 2 周 |

### 13.2 实施路线图

#### Week 1-2: Git 集成系统

**目标**: 实现结构化 Git 状态展示、diff 解析、智能提交建议

```
Week 1
├─ Day 1-2: Git 状态解析
│   ├─ 实现 GitExecutor（调用系统 git 命令）
│   ├─ 解析 git status --porcelain=v2
│   └─ 结构化输出（分支、变更文件、统计）
│
├─ Day 3-4: Diff 解析
│   ├─ 解析 git diff 输出
│   ├─ 提取函数级变更（added/modified/deleted）
│   └─ 生成变更摘要
│
└─ Day 5: CLI 命令
    ├─ /git status 命令
...
Week 2
├─ Day 1-2: 提交建议生成
│   ├─ 基于规则的简单生成（type + scope）
│   ├─ 基于 LLM 的智能生成
│   └─ Conventional Commits 格式
│
├─ Day 3-4: 安全提交流程
│   ├─ 提交预览（变更摘要 + diff）
│   ├─ 用户确认（Y/n/edit/cancel）
│   └─ 回滚支持
│
└─ Day 5: 测试与优化
    ├─ 单元测试（解析器）
...
**验收标准**:
- [ ] `/git status` 显示结构化状态（分支、变更文件、统计）
- [ ] `/git suggest` 生成符合 Conventional Commits 的提交信息
- [ ] `/git commit` 带预览和确认，支持取消
- [ ] 提交信息准确率 > 80%（与人工编写对比）
- [ ] 支持中文提交信息生成

#### Week 3-4: 代码索引系统

**目标**: 实现轻量级符号索引，支持定义查找、调用者搜索、影响分析
...
├─ Day 3-4: CLI 命令
│   ├─ /index build 构建索引
│   ├─ /index find <symbol> 查找定义
│   ├─ /index callers <func> 查找调用者
│   └─ /index impact <func> 影响分析
│
└─ Day 5: 测试与优化
    ├─ 性能测试（100 个文件 < 5s）
    ├─ 准确率测试（找到 90%+ 调用）
...
**验收标准**:
- [ ] `/index build` 在 5 秒内完成 100 个文件的索引
- [ ] `/index find <symbol>` 准确找到定义位置
- [ ] `/index callers <func>` 找到 90% 以上的调用位置
- [ ] `/index impact <func>` 分析直接调用和间接调用
- [ ] 索引存储 < 1MB/1000 个函数
- [ ] 支持增量更新（只更新变更文件）
...
**验收标准**:
- [ ] Agent 能回答"这个函数在哪里被调用"
- [ ] Agent 能建议"修改此接口会影响 X 个地方"
- [ ] Git 提交时自动显示变更影响
- [ ] 整体性能满足基准要求
...
```
现有迁移计划（Phase 1-15）: 架构重构 + 增强能力
P0 改进计划（Week 1-5）:    功能补齐（可并行）

建议执行方式:
1. 架构重构（Phase 1-8）和 P0 改进可并行开发
2. P0 改进基于现有 services/ 代码实现
3. 架构重构完成后，将 P0 代码迁移到 internal/
```
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| 正则解析不准确 | 中 | 中 | 逐步优化 + 用户反馈 |
| 大项目索引慢 | 中 | 中 | 增量更新 + 并行构建 |
| LLM 提交建议质量差 | 低 | 中 | 规则兜底 + 用户编辑 |
| 与现有代码冲突 | 低 | 高 | 独立模块 + 接口隔离 |

### 13.6 成功指标

实现 P0 改进后，与 Claude Code 的能力对比：

| 能力 | 改进前 | 改进后 | Claude Code | 差距 |
|------|--------|--------|-------------|------|
| Git 集成 | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 接近 |
| 代码导航 | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 接近 |
| 变更分析 | ⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 缩小 |
...
**核心优势仍然保持**：
- ✅ 纯本地运行（Claude Code 无法做到）
- ✅ 零外部依赖（Claude Code 无法做到）
- ✅ 完全开源可控（Claude Code 无法做到）
- ✅ 离线可用（Claude Code 无法做到）

---

## 14. 大型项目支持计划

> 目标: 支持 10K+ 文件、Monorepo、多模块工作区
> 优先级: P1
> 预计周期: 3 周
...
| 阶段 | 功能 | 时间 | 依赖 |
|------|------|------|------|
| **Phase 1** | Workspace 自动识别 + 配置 | 3 天 | Git 集成完成 |
| **Phase 2** | 分层索引架构 | 5 天 | Phase 1 |
| **Phase 3** | 增量索引 + 变更检测 | 3 天 | Phase 2 |
| **Phase 4** | LRU 内存缓存 | 3 天 | Phase 3 |
| **Phase 5** | 并行索引 | 2 天 | Phase 4 |
| **Phase 6** | 分层上下文注入 | 4 天 | Phase 5 |
| **Phase 7** | 性能测试 + 优化 | 3 天 | Phase 6 |

### 14.2 验收标准

- [ ] 自动识别 Go Workspace / Node Monorepo / Rust Workspace
- [ ] 10K 文件全量索引 < 30s
- [ ] 100 文件增量更新 < 3s
- [ ] 索引内存占用 < 256MB（可配置）
- [ ] 跨模块符号搜索 < 200ms
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| 文件监控性能差 | 中 | 高 | 使用轮询 + 指纹，非实时监控 |
| 索引文件过大 | 中 | 中 | 分层存储 + 压缩 |
| 跨语言符号冲突 | 低 | 中 | 模块隔离 + 前缀命名 |
