...
### 自主学习——越用越聪明
- **统一查询分析**: 单次 LLM 调用完成任务分类+技能匹配+复杂度估算+后处理规划
- **LLM 自主决策**: LLM 自己判断什么值得学——无固定阈值
- **过程监督**: 逐轮次评估，精确定位最佳和最差决策
- **质量门控 (LLM-first)**: LLM 反思评分 → 直问 LLM → 公式兜底
- **知识精炼**: 去重 → LLM 结构化提取 → 复合评分检索（相关性+重要性+时效性）
- **技能蒸馏**: 语义/LLM 聚类 → evaluateAndDistill（单次 LLM） → 自动创建 slash command
- **思维树探索**: 决策点多路径并联探索，LLM 评估选最优方案
- **ProjectMind**: 跨会话项目知识积累
- **配置开关**: `distill_enabled` + `knowledge_enabled` — 热重载无需重启
